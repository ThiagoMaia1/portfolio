...
const estilosSeta : Array<'estiloSetaUm'|'estiloSetaDois'> = ['estiloSetaUm', 'estiloSetaDois'];
...
class Carrossel extends Component<Props, State> {
    ...
    constructor (props : Props) {
        ...
        this.style = {};
        this.style['max' + capitalize(this.dimensoes.principal, 'Primeira Maiúscula')] = props.tamanhoMaximo
        this.estiloSeta[this.dimensoes.principal] = '10%';
        this.estiloSeta[this.dimensoes.secundaria] = '100%';
        let obj : State = {tamanhoCarrossel: 0, tamanhoGaleria: 0, mouseEnterSeta: 0, estiloSetaUm: {}, estiloSetaDois: {}, estiloGaleria: {}};
        for (var i = 0; i < this.direcao.length; i++) {
            obj[estilosSeta[i]] = {
                backgroundImage: 'linear-gradient(to ' + Direcao[this.direcao[i]] + ', rgba(0,0,0,0), ' + this.corGradiente + ')', 
                opacity: 0,
            };
        ...
        this.state.estiloGaleria[Direcao[this.direcao[0]] as keyof CSSProperties] = '0';
    }
    ...
    getOffsetAtual = () => Number(this.state.estiloGaleria[Direcao[this.direcao[0]] as keyof CSSProperties]);
    getPasso = (sentido : IndexSeta, tamanhoPasso : number) => - (sentido ? 1 : -1)*tamanhoPasso;
    eSetaInvisivel = (iSeta : IndexSeta) => this.state[estilosSeta[iSeta]].opacity === 0;
    getDimensaoCamel = (palavra : string) => palavra + capitalize(this.dimensoes.principal, 'Primeira Maiúscula');
    ...
    definirEstiloSeta = (iSeta = 2, opacidade ?: number, display ?: boolean) => {
        ...
        if (nomeEstiloSeta) {
            ...
            if (displayStr !== undefined) objState[nomeEstiloSeta].display = displayStr;
    ...
    definirOffset(passo : number, opacidadeSetas = true) {
        ...
        objEstilo[Direcao[this.direcao[0]]] = 
        Math.min(
            Math.max(
    ...
    encontrarSelecionado = (elemento : HTMLElement) => {
        ...
        var coordElemento : number =  getCoords(elemento)[Direcao[this.direcao[0]] as keyof Coordinates];
        var carrossel = this.refCarrossel.current as HTMLElement;
        var coordCarrossel : number = getCoords(carrossel)[Direcao[this.direcao[0]] as keyof Coordinates];
        const espacoExtra = 30;
        var distancia = coordCarrossel > coordElemento
                        ? Math.max(coordCarrossel - coordElemento + espacoExtra, 0)
                        : Math.min(coordCarrossel + Number(carrossel[dimensao as keyof HTMLElement]) - 
                            coordElemento - Number(elemento[dimensao as keyof HTMLElement]) - espacoExtra, 0);
    ...
    definirDisplaySetas = () => {
        this.timeoutDisplaySetas = setTimeout(() => {
            ...
            var tamanho = galeria ? galeria[('offset' + capitalize(Direcao[this.direcao[0]], 'Primeira Maiúscula')) as keyof HTMLElement] ?? 0 : 0;
            var posicao = tamanho;
    ...
    offsetComTransition(distancia : number, taxaTransition = 300, opacidadeSetas = true) {
    ...
    detectarTamanho = () => {
        ...
        if (this.state.tamanhoCarrossel >= this.state.tamanhoGaleria && this.state.estiloGaleria[Direcao[this.direcao[0]]] !== '0') {
            var objEstilo : Coordinates = {};
                    ...
                    ? setas.map((s, i) => {
                        ...
                        return (
                            <div className="seta-galeria" 
                                ...
                                style={{...this.estiloSeta, ...this.state[estilosSeta[j]], ...this.estiloFixed}}
                                key={i}>
                                <div className='arrow-circle' 
                                     style={{paddingTop: padArrowCircle, ...(j ? {paddingLeft: padArrowCircle} : {paddingRight: padArrowCircle})}}>
