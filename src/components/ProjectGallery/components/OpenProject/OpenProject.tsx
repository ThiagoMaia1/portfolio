...
const OpenProject = ({ project, onClick }: Props) => {
  const [ativo, setAtivo] = useState(true)
  const [scrollOutside, setScrollOutside] = useState(0)
  const timeoutScroll = useRef(setTimeout(() => void 0, 0))
  useEffect(() => {
  ...
  const style = useClosingAnimation(
    ...
    {
      transform: 'perspective(10px) rotateX(86.8deg)',
      top: '-50%',
      opacity: 0,
    },
    time,
  ...
  return (
    <div className="open-project-wrapper">
      <div className="shadow-open-project-animation">
        <div
          className="open-project project-card"
          style={{
            ...style,
            transition: style.transition + ', ' + opacityTransition,
          }}
        />
      </div>
      <div
