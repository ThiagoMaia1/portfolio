...
export class WavePath implements WaveData {
    ...
    constructor(waveData : WaveData, scope : paper.PaperScope) {
        ...
        switch (this.side) {
            case Sides.up:
                ...
                this.path.add(c.bottomRight.add(sideDifPoint), c.bottomCenter, c.bottomLeft.subtract(sideDifPoint));
                break;
            case Sides.both:
                this.createWavePoints(0.2);
                this.path.reverse();
            ...
            case Sides.down:
                ...
                this.path.add(c.topRight.add(sideDifPoint), c.topCenter, c.topLeft.subtract(sideDifPoint));
                this.createWavePoints(1);
                break;
        }
    }
    ...
    createWavePoints = (lineHeight : number) : void => {
        for (let i = 0; i < this.points; i++) {
            this.path
                .add(new this.scope.Point(i / (this.points-1), 1)
                .multiply(this.container.width + this.sideWidth*2)
                .subtract(new this.scope.Point(this.sideWidth, 0)));
        }

        let segmentNumber = this.path.segments.length;
        let initialPoints = segmentNumber - this.points;
        const heights = Array.from({length: this.points}, () => Math.floor(Math.random() * this.randomStrenghtFactor * 100));
        
        this.onFrameFunctions.push((event : OnFrameEvent) => {
            for (let i = initialPoints; i < segmentNumber; i++) {
                let segment = this.path.segments[i];
                let sinus = Math.sin(this.animationFunction(event, i));
                segment.point.y = this.container.y + sinus * this.height + this.container.height*lineHeight + i*this.inclinationFactor + heights[i - initialPoints];
            }
