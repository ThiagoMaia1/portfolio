    ...
    <div className="background waves">
      <PaperElement
        ...
        animation={(scope) => {
          ...
          const waves = Array(numberOfWaves)
            .fill(0)
            .map(
              (_, i) =>
                new WavePath(
                  {
                    container: new scope.Rectangle(
                      new scope.Point(0, view.bounds.height * 0.005 * (1 + i)),
          ...
          waves.forEach((w) => {
            w.path.strokeColor = new scope.Color(
              getGradient(
                new scope.Rectangle(
                  view.bounds.topCenter,
                  view.bounds.bottomCenter,
                ),
                ['#ff0000', 'transparent'],
                scope,
              ),
          ...
          waves.forEach((w) => {
            w.onFrameFunctions.forEach((f) =>
              f({ time: 0, delta: 0, count: 0 }),
            )
