...
import technologies, {
  getTechnologiesOfFamily,
} from '../../../models/Technology/Technologies'
import useHashTechnologies, {
  hashSeparator,
} from '../../../hooks/useHashTechnologies'
...
function SeeTechnologyFamily({ setSearchTerm }: Props) {
  ...
  const onClick = () => {
    ...
    setSearchTerm(
      familyTechnologies.map((t) => technologies[t].name).join(' | '),
    )
  }
  ...
  return (
    <AppearFromBelow>
      <Div>
        <Button onClick={onClick}>{$t('relatedTechnologies')}</Button>
      </Div>
    </AppearFromBelow>
  )
}
